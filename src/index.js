(async function main() {
  ...
  const getDimensions = () => {
    ...
    w.addEventListener('resize', /** @param {Event} evt */(evt) => {
  ...
  function printFrame(frame) {
    ...
    const [incrementX, incrementY] = [(widthIsTooLarge ? width : widthForRatio) / frameWidth, (heightIsTooLarge ? height : heightForRatio) / frameHeight]

    for (const [i, row] of Object.entries(frame)) {
      for (const [j, pixel] of Object.entries(row)) {
        const [posX, posY] = [j * incrementX, i * incrementY]
        ctx.fillText(pixel, posX + (widthIsTooLarge ? 0 : (Math.abs(width - widthForRatio) / 2)), posY + (heightIsTooLarge ? 16 : (Math.abs(height - heightForRatio) / 2) + 16))
      }
    }
  }
  ...
  function playVideo(array, startTime = Date.now()) {
    try {
      const currentFrameCount = Math.min(Math.round((Date.now() - startTime) / (1000 / FRAMERATE)))
  ...
  const loadBadApple = async () => {
    const badApple = await fetch('/src/assets/badApple.zip').then(r => r.blob())
    const badAppleAB = await badApple.arrayBuffer()

    const zip = JSZip();

    const badAppleZip = await zip.loadAsync(badAppleAB)
    const badAppleStringified = await badAppleZip.file('badApple.json').async('string')
